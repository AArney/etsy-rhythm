<div class="wrap">
			...
			<h3><?php _e('General Settings', 'etsyrhythm');?></h3>
                ...
                <tr valign="top">
                    <th scope="row">
                        <label for="api_key"></label><?php _e('Etsy API Key', 'etsyrhythm'); ?>
                    </th>
                    <td>
                        <input id="etsy_rhythm_settings[api_key]" name="etsy_rhythm_settings[api_key]" type="text" size="25" value="<?php echo $options['api_key']; ?>" class="regular-text code" />
                            ...
                            <?php /* } */ ?>
                            <p class="description">
								<?php echo sprintf( __('You may get an Etsy API Key by <a href="%1$s">Creating a new Etsy App</a>', 'etsyrhythm' ), 'http://www.etsy.com/developers/register' ); ?>
							</p>
                    </td>
                 </tr>
				...
				?>
                 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[target_blank]"></label><?php _e('Link to new window', 'etsyrhythm'); ?>
					</th>
                    <td>
						<input id="etsy_rhythm_settings[target_blank]" name="etsy_rhythm_settings[target_blank]" type="checkbox" value="1" <?php if (isset($options['target_blank'])) { checked('1', $options['target_blank']); } ?> />
							<p class="description">
                               <?php echo __( 'If you want your links to open a page in a new window', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
				...
				?>
                 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[cache_life]"></label><?php _e('Cache life', 'etsyrhythm'); ?>
                    ...
                    <td>
                        <input id="etsy_rhythm_settings[cache_life]" name="etsy_rhythm_settings[cache_life]" type="text" size="25" value="<?php echo $options['cache_life']; ?>" class="regular-text code" />
                            <p class="description">
                                <?php echo __( 'How long until the cache file updates. If you are unsure, leave this alone. Default is set at 6 hours or 21600 seconds.', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
				...
				?>
                 <tr valign="top">
                    <th scope="row"><?php _e('Cache Status', 'etsyrhythm'); ?></th>
                        <td>
                            ...
                            <table class="wp-list-table widefat fixed">
                                ...
                                foreach ($files as $file) {
                                    ...
                                    ?></table><?php } else { _e('You must enter your Etsy API Key to view cache status!', 'etsyrhythm'); } ?>
                                <p class="description"><?php _e( '', 'etsyrhythm' ); ?></p>
                                </td>
                </tr>
				...
				?>
                 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[reset_cache]"></label><?php _e('Reset Cache', 'etsyrhythm'); ?>
					</th>
                    <td>
						<input id="etsy_rhythm_settings[reset_cache]" name="etsy_rhythm_settings[reset_cache]" type="checkbox" value="1" <?php if (isset($optons['reset_cache'])) { checked('0', $options['reset_cache']); } ?> />
							<p class="description">
                               <?php echo __( 'Reset Cache', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
             </table>
             ...
             <table class="form-table">
             <h3><?php _e('Look & Feel', 'etsyrhythm');?></h3>    
				 <?php 	/* 
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[quantity]"></label><?php _e('Number of items to list', 'etsyrhythm'); ?>
					</th>
                    <td>
                        <input id="etsy_rhythm_settings[quantity]" name="etsy_rhythm_settings[quantity]" type="text" size="25" value="<?php echo $options['quantity']; ?>" class="regular-text code" />
                            <p class="description">
                                <?php echo __( 'How many items to list ( 1 - 25 )', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
                 <?php 	/* 
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[title_length]"></label><?php _e('Length of Item Title ( in characters )', 'etsyrhythm'); ?>
					</th>
                    <td>
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[user_rows]"></label><?php _e('Length of Rows', 'etsyrhythm'); ?>
					</th>
                    <td>
                        <input id="etsy_rhythm_settings[user_rows]" name="etsy_rhythm_settings[user_rows]" type="text" size="25" value="<?php echo $options['user_rows']; ?>" class="regular-text code"/>
                            <p class="description">
                                <?php echo __( 'Example: a setting of 1 will only display 1 item per row.', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[materials]"></label><?php _e('Display materials used in making item', 'etsyrhythm'); ?>
					</th>
                    <td>
                        <input id="etsy_rhythm_settings[materials]" name="etsy_rhythm_settings[materials]" type="checkbox" value="1" <?php if (isset($options['materials'])) { checked('1', $options['materials']); } ?> />
                            <p class="description">
                                <?php echo __( 'For instance if the creator used oil paint, this will show "Oil paints"', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[who_made]"></label><?php _e('Display who made the item', 'etsyrhythm'); ?>
					</th>
                    <td>
				...
				?>
				 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[when_made]"></label><?php _e('Display when the item was made', 'etsyrhythm'); ?>
					</th>
                    <td>
                        <input id="etsy_rhythm_settings[when_made]" name="etsy_rhythm_settings[when_made]" type="checkbox" value="1" <?php if (isset($options['when_made'])) { checked('1', $options['when_made']); } ?> />
                            <p class="description">
                                <?php echo __( 'example: July 2002', 'etsyrhythm' ); ?>
                            </p>
                    </td>
                 </tr>
				 <?php 	/* 
				...
				?>
                 <tr valign="top">
                     <th scope="row">
                         <label for="etsy_rhythm_settings[language]"></label><?php _e('Language', 'etsyrhythm'); ?>
					</th>
                    <td>
                        <select id="etsy_rhythm_settings[language]" name='etsy_rhythm_settings[language]'>
                            ...
                            <p class="description">
                                <?php echo __( 'Select a language to view items in. Note that not all shops supply translations, so even though you select French, you may only get English.', 'etsyrhythm' );;?>
                            </p>
                    </td>
                 </tr>
        </table>
